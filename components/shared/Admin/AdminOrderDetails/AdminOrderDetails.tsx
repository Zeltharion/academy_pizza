...
export const AdminOrderDetails: React.FC<IAdminOrderDetails> = ({
	...
	return (
		<div className={cn(s.adminOrderDetails, className)}>
			<div className={s.adminOrderDetails__main}>
				<div className={s.adminOrderDetails__main__title}>
					<Title
						text={`Заказ #${values.id}`}
						size="xl"
						className="font-extrabold"
					/>
					<Badge className={cn(s.adminOrderDetails__main__status, {
				...
				<div className={s.adminOrderDetails__main__personalInfo}>
					...
					{values.comment && <span>Комментарий: <b>{values.comment}</b></span>}
				</div>

				{items.map(item => (
					<article className={s.adminOrderDetails__main__products__wrapper} key={item.id}>
						<img
							src={item.productVariant.product.imageUrl}
							alt={item.productVariant.product.name}
							className={s.adminOrderDetails__main__products__image}
						/>
						<div className={s.adminOrderDetails__main__products}>
							<div className={s.adminOrderDetails__main__products__title_wrapper}>
								<Title
									text={item.productVariant.product.name}
									size="sm"
									className={s.adminOrderDetails__main__products__title}
								/>
							...
							{item.ingredients && item.ingredients.length > 0 && (
								<div className={s.adminOrderDetails__main__products__ingredients}>
									{item.ingredients.map((ingredient) => (
										<div key={ingredient.id} className={s.adminOrderDetails__main__products__ingredients__item}>
											<img src={ingredient.imageUrl} alt={ingredient.name} />
											<span className="text-sm">
												+{formatNumberToMoney(ingredient.price)}
											</span>
										</div>
									))}
								</div>
							)}
							<div className={s.adminOrderDetails__main__products__price}>
								<b>Кол-во:{" "}
									<span className={s.adminOrderDetails__main__products__price__quantity}>
										{item.quantity} шт.
									</span>
								</b>
								<b>Цена:{" "}
									<span className={s.adminOrderDetails__main__products__price__priceWithQuantity}>{
